...
namespace kdt
{
	/* @BruceWayne-Batty k-d tree class.*/
	template <class Photon>
	class KDTree
	{
	public:
		/* @BruceWayne-Batty The constructors.*/
		KDTree() : root_(nullptr) {};
		KDTree(const std::vector<Photon>& points) : root_(nullptr) { build(points); }

		/* @BruceWayne-Batty The destructor.*/
		~KDTree() { clear(); }

		/* @BruceWayne-Batty Re-builds k-d tree.*/
		...
		/* @BruceWayne-Batty Clears k-d tree.*/
		void clear()
		{
		...
		/* @BruceWayne-Batty Validates k-d tree.*/
		bool validate() const
		{
			try
			{
		...
		/*@BruceWayne-Batty Searches the nearest neighbor.*/
		int nnSearch(const Photon& query, double* minDist = nullptr) const
		{
			int guess;
		...
		/* @BruceWayne-Batty Searches k-nearest neighbors.*/
		std::vector<int> knnSearch(const Photon& query, int k) const
		{
			KnnQueue queue(k);
		...
		/* @BruceWayne-Batty Searches neighbors within radius.*/
		std::vector<int> radiusSearch(const Photon& query, double radius) const
		{
	...
	private:
		...
		/* @BruceWayne-Batty k-d tree node.*/
		struct Node
		{
		...
		/* @BruceWayne-Batty k-d tree exception.*/
		class Exception : public std::exception { using std::exception::exception; };

		/*@BruceWayne-Batty Bounded priority queue.*/
		...
		/* @BruceWayne-Batty Priority queue of <distance, index> pair.*/
		using KnnQueue = BoundedPriorityQueue<std::pair<double, int>>;

		/* @BruceWayne-Batty Builds k-d tree recursively.*/
		...
		/* @BruceWayne-Batty Clears k-d tree recursively.*/
		...
		/* @BruceWayne-Batty Validates k-d tree recursively.*/
		...
		{
			double dist = 0;
			for (size_t i = 0; i < Photon::DIM; i++)
				dist += (p[i] - q[i]) * (p[i] - q[i]);
			return sqrt(dist);
		}

		/* @BruceWayne-Batty Searches the nearest neighbor recursively.*/
		void nnSearchRecursive(const Photon& query, const Node* node, int* guess, double* minDist) const
		{
			if (node == nullptr)
				return;

			const Photon& train = points_[node->idx];
		...
		/* @BruceWayne-Batty Searches k-nearest neighbors recursively.*/
		void knnSearchRecursive(const Photon& query, const Node* node, KnnQueue& queue, int k) const
		{
			if (node == nullptr)
				return;
		...
		/* @BruceWayne-Batty Searches neighbors within radius.*/
		void radiusSearchRecursive(const Photon& query, const Node* node, std::vector<int>& indices, double radius) const
		{
			if (node == nullptr)
				return;
		...
		std::vector<Photon> points_; //!< points
	};
} // kdt
